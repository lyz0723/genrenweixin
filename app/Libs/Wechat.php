...
class Wechat
{
	...
	public function __construct($options)
	{
		...
		$this->payKey = isset($options['payKey'])?$options['payKey']:false;
		$this->merchantId = isset($options['merchantId'])?$options['merchantId']:false;
		$this->paySSLCert = isset($options['paySSLCert'])?$options['paySSLCert']:false;
		$this->paySSLKey = isset($options['paySSLKey'])?$options['paySSLKey']:false;
		$this->templateMsg = isset($options['templateMsg'])?$options['templateMsg']:false;
        $this->isServiceProvider = isset($options['isServiceProvider'])?$options['isServiceProvider']:false;
        $this->qrcode = isset($options['qrcode'])?$options['qrcode']:null;
     ...
     * 设置支付商户号信息
     */
    public function setPayMerchantInfo($merchantInfo)
    {
	...
	protected function logcallback($log)
	{
		Log::info($log);
	}
	...
	public function getRevEventGeo(){
        	if (isset($this->_receive['Latitude'])){
        		 return array(
				'x'=>$this->_receive['Longitude'],
				'y'=>$this->_receive['Latitude'],
	...
	*/
    ...
    public function xml_to_array($doc)
    {
    	$xml = simplexml_load_string($doc, 'SimpleXMLElement', LIBXML_NOCDATA);
        return $this->simplexml_to_array($xml);
    }

    public function simplexml_to_array($xml, $result = [])
    {
        foreach ((array)$xml as $name => $child) {
            if (is_object($child)) {
                $result[$name] = $this->simplexml_to_array($child);
            } else {
                $result[$name] = $child;
            }
        }
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		$expiredMins = $expired / 60;
		return Cache::put($cachename, $value, $expiredMins);
	}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $lock = 'wechat_access_token_'.$appid.'_lock';
        if ($lockRes = Cache::add($lock, 1, 0.5))
        {
            $this->resetAuth();
            ...
            if ($result)
            {
                ...
                Log::error("WechatSdkGetAccessTokenResponse\t{$result}");
                ...
                else
                {
                    ...
                    $expire = $json['expires_in'] ? intval($json['expires_in'])-1200 : 3000;
	...
	public function getUnifiedOrder($body, $tradeNo, $totalFee, $clientIp, $notifyUrl, $openid, $goodsTag = '') {
        ...
        if ($this->isServiceProvider)
        {
            $data['appid'] = env('WECHAT_SERVICE_PROVIDER_APPID');
            $data['mch_id'] = env('WECHAT_SERVICE_PROVIDER_MCHID');
            $data['sub_appid'] = $this->appid;
            $data['sub_mch_id'] = $this->merchantId;
            $data['sub_openid'] = $openid;
            unset($data['openid']);
            $this->payKey = env('WECHAT_SERVICE_PROVIDER_PAYKEY');
        }

        Log::info('UnifiedOrderData:' . json_encode($data));
        ...
        Log::info("Wechat.getUnifiedOrder\t" . json_encode($data));
    ...
    public function getOrder($orderno)
    {
        ...
        $data['sign'] = $this->getPaySign($data, $this->payKey);
        Log::info("WechatOrderGetRequest\t" . json_encode($data));
        $url = 'https://api.mch.weixin.qq.com/pay/orderquery';
    	$post = $this->xml_encode($data);
    	$res = $this->http_post($url, $post);

        $res = $this->xml_to_array($res);
        Log::info("WechatOrderGetResponse\t" . json_encode($res));
    	return $res;
    }
    ...
    public function closeorder($orderno)
    {
        ...
        $data['sign'] = $this->getPaySign($data, $this->payKey);
        Log::info("WechatOrderCloseRequest\t" . json_encode($data));
        $url = 'https://api.mch.weixin.qq.com/pay/closeorder';
    	$post = $this->xml_encode($data);
    	$res = $this->http_post($url, $post);

    	$res = (array)simplexml_load_string($res, 'SimpleXMLElement', LIBXML_NOCDATA);
        Log::info("WechatOrderCloseResponse\t" . json_encode($res));
    	return $res;
    }

	public function refund($transactionId, $outRefundNo, $totalFee, $refundFee) {
    	...
    	$post = $this->xml_encode($data);
    	$res = $this->http_post($url, $post, false, true);
    	$res = (array)simplexml_load_string($res, 'SimpleXMLElement', LIBXML_NOCDATA);
    	return $res;
	}

	public function refundQuery($transactionId)
	{
		$data = [
    		'appid' => $this->appid,
    		'mch_id' => $this->merchantId,
     ...
     * @return array
     */
    public function sendCoupon($openid, $stockId, $tradeNo)
    {
        $data = [
     ...
     * @return array
     */
    public function sendLottoCoupon($openid, $activityId, $tradeNo, $src, $batchId = null)
    {
        $data = [
            'src' => $src,
            'openid' => $openid,
            'idempotent_str' => md5($tradeNo),
            'noncestr' => $this->generateNonceStr(),
            'timestamp' => time(),
        ...
        $url = 'https://pay.weixin.qq.com/cgi-bin/lotto/sendcoupon?';
        $url .= implode('&', array_reduce(array_keys($data), function($result, $key) use ($data) {
            $result[] = $key . '=' . urlencode($data[$key]);
            return $result;
        }, []));
        ...
        $res = json_decode(substr(ltrim($res, ';'), strpos($res, '(') + 1, -1));
        if ($res['error_code'])
        {
            $this->errCode = $res['error_code'];
	 ...
	 * 自动提款接口
	 *
	 * @param string $tradeNo 提现订单号
	 * @param string $openid 提现收款账号openid
	 * @param integer $amount 提现金额,单位分
	 * @param string $desc 提现描述
	 * @param string $clientIp 付款ip
	 * @return array
     */
    public function automaticWithdraw($tradeNo, $appid, $openid, $amount, $desc, $clientIp)
    {
    	$data = [
    		'mch_appid' => $appid,
    	...
    	$post = $this->xml_encode($data);
    	$res = $this->http_post($url, $post, false, TRUE);
    	Log::info("WechatSdkAutomaticWithdraw\t" . json_encode($data));
    	$res = (array)simplexml_load_string($res, 'SimpleXMLElement', LIBXML_NOCDATA);
    	return $res;
    }
     ...
     * 拉取企业付款订单
     *
     * @param string $tradeNo 企业付款订单号
     * @param array 
     */
    public function getWithdraw($tradeNo)
    {
        $data = [
            'appid' => env('WECHAT_BUSINESS_MP_APPID'),
            'mch_id' => $this->merchantId,
            'partner_trade_no' => $tradeNo,
            'nonce_str' => $this->generateNonceStr()
        ];
    	...
    	$res = $this->http_post($url, $post, false, TRUE);
    	$res = (array)simplexml_load_string($res, 'SimpleXMLElement', LIBXML_NOCDATA);
    	Log::info("WechatSdkGetWithdraw\trequest:" . json_encode($data) . "\tresponse:" . json_encode($res));
    	return $res;
    }
    ...
    public function batchGetUserInfo($openids)
    {
        ...
        {
            Log::error("WechatSdkBatchGetUserInfo\tOutOfPerBatchlimit");
            return false;
        }
        ...
        {
            $userList['user_list'][] = ['openid' => $openid];
        }

		$result = $this->http_post(self::API_URL_PREFIX.self::USER_INFO_BATCHGET_URL.'access_token='.$this->access_token, json_encode($userList));
		if ($result)
		{
			$json = json_decode($result, true);
			if (isset($json['errcode'])) {
